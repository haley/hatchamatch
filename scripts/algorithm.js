...
var bigs = window.localStorage.getItem('bigsnames').split(",");
var littles = window.localStorage.getItem('littlesnames').split(",");
...
for(big=0; big<bigs.length; big++) {
  var bigsPrefs = window.localStorage.getItem(bigs[big]).split(",");
  people[bigs[big]].add_prefs(bigsPrefs);
}


for(little=0; little<littles.length; little++) {
  var littlesPrefs = window.localStorage.getItem(littles[little]).split(",");
  people[littles[little]].add_prefs(littlesPrefs);
}
...
function crossover(list1, list2, last_set_index) {
  ...
  end = Math.floor(Math.random()*(last_set_index - start) + start + 1);
...
function breed(start_pop, generation, last_set_index, stat_group){
  ...
  var min = littles.length*bigs.length*3;
  var min_val = null;
  ...
  var gen_min = littles.length*bigs.length*3;
  var gen_min_val = null;
  ...
  for(person=0; person<stat_group.length; person++){
    if(min_val[person] !== null) {
      if(stat_group[person] in final_matchings) {
        final_matchings[stat_group[person]].push(min_val[person]);
      } else {
        final_matchings[stat_group[person]] = [min_val[person]];
      }
    }
  }
}
...
function create_starting_pop(stat_group, shuffle_group, last_set_index) {
  ...
  for (pop=0; pop<population; pop++) {
    ...
    if (heap.size() < population/20) {
      heap.push(matching);
    } else if (heap.peek().score > score) {
      heap.pushpop(matching);
    }
  }
  generations = bigs.length + littles.length * 30;
  cull = heap.toArray();
  breed(cull, generations, last_set_index, stat_group);
...
function setup(bigs, littles) {
  ...
  if (bigs.length > littles.length) {
  ...
  } else if (littles.length > bigs.length) {
    ...
    var stat_group = bigs.slice(0);
    var shuffle_group = littles.slice(0);
    var last_set_index = littles.length;
  }
  if (stat_group.length % shuffle_group.length === 0) {
    last_set_index = shuffle_group.length;
  }
  ...
  for(i=0; i<bigs.length; i++){
    ...
    for(j=0; j<littles.length; j++){
      ...
      if (fitness_matrix[[big.name, little.name]] == 0) {
        if (bigs.length > littles.length) {
          final_matchings[little.name] = [big.name];
        } else {
          final_matchings[big.name] = [little.name];
        }
        last_set_index--;
        if (stat_group.includes(big.name)) {
        ...
        } else if(stat_group.includes(little.name)) {
